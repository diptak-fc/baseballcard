...
// GET: full roster with assignments (Admin + CEO)
...
// POST: add a new CSM/CSA (Admin)
export async function POST(req: NextRequest) {
  try {
    await requireRole("ADMIN");
    const { name, email, title, password } = await req.json();
    if (!name || !email) {
    ...
    const rows = await sql`
      ...
      VALUES (${String(email).toLowerCase()}, ${name}, ${hash}, 'CSM', ${title || "CSM"})
